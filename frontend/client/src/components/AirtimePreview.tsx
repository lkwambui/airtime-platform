  ...
  return (
    <div className="relative group">
      ...
      <div className={`relative ${theme === 'dark' ? 'bg-linear-to-br from-slate-700 to-slate-800' : 'bg-linear-to-br from-gray-100 to-gray-50'} ${theme === 'dark' ? 'border-slate-600/50' : 'border-gray-200/50'} border rounded-2xl p-6 text-center space-y-2`}>
        <p className={`text-sm font-medium ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'} uppercase tracking-wider`}>You will receive</p>
        ...
        <p className={`text-xs font-semibold ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'}`}>in airtime credit</p>
      </div>
    </div>
  );
}
